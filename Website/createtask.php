...
<html>
	...
	<body>
		<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
			...
			<div class="collapse navbar-collapse" id="navbarColor01">
                ...
                <li class="nav-item active">
                    <a class="nav-link" href="createtask.php">CreateTask <span class="sr-only">(current)</span></a>
                </li>
                <li class="nav-item">
        ...
        <form role='form' class='form-signin' action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
                ...
                <button id='createtaskbtn' class="btn" type="submit" name='createtask' >Create</button>
		</form>
        ...
        <?php
            ...
            if($_SERVER["REQUEST_METHOD"] == "POST")
            {
                ...
                if (empty($taskname) or empty($projectname) or empty($description))
                {
                    die ("<div class='alert alert-dismissible alert-warning'>
                    <button type='button' class='close' data-dismiss='alert'>&times;</button>
                    <strong>Oh snap!</strong> Please fill all the fields! </div>");
                }

                $attributes = array("month"   => $_POST["month"], 
                "day"   => $_POST["day"],
                "year" => $_POST["year"]
                );
                $deadline = $attributes["month"] . "/" . $attributes["day"] . "/" . $attributes["year"];

                $q = "EXEC CreateTask '$manager', '$taskname', '$projectname', '$deadline', '$description'";
                ...
                if ($rows_affected > 0)
                {
                    ...
                    <button type='button' class='close' data-dismiss='alert'>&times;</button>
                    <strong> Success!</strong> Task created!</div>";
                }
                else
                {
                    echo "<div class='alert alert-dismissible alert-danger'>
                    <button type='button' class='close' data-dismiss='alert'>&times;</button>
                    <strong>Oh snap!</strong> could not create task! </div>";
                }
            }
        ?>
    </body>
